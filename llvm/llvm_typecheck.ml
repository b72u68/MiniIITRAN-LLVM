...
let rec typecheck_inst ?(enforce_ssa = false) 
  ...
  | IRet None -> ts
  | ISwitch (((TInteger _) as t), v, _, cases) ->
     (compat_with_type t v;
      if List.for_all
           (function (t', _, _) -> teq t t')
           cases
      then ts
      else raise (TypeError ("Case doesn't have correct type", i))
     )
  | ISwitch _ -> raise (TypeError ("Can only switch on integer", i))
  | ICall (d, t, v, args) ->
     (try
