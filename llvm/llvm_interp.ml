...
let rec interp ((p, ctx): prog * typdefs) (globs: value Varmap.t) =
  ...
  let rec interp_int
    ...
    | IRet None -> (globs, 0)
    | IRet (Some (_, v)) ->
       (globs, interp_value v)
    | ISwitch (_, v, defl, labels) ->
       let v = interp_value v in
       let l =
         try let (_, _, l) = List.find (fun (_, v', _) -> v = v') labels
             in l
         with Not_found -> defl
       in do_branch l
    | ICall (d, _, f, args) ->
       (match (f, args) with
        ...
        | _ ->
           ...
           let (globs, res) = interp_int func "" 0 newregs globs newstack heap
           in
